...
%The main function for calculating the key rate of TP-TFQKD under symmetric channel

la=l/2;   %l is the total distance between Alice and Bob
lb=l/2;
pd=10^-8;
fec=1.1;
edz=0;
...
pnub=pnua;
pobd=poad;
poap=1-pmua_tot-pnua-poad;
pobp=1-pmub-pnub-pobd;
poa=poad+poap;
pob=pobd+pobp;
pood=poa*pob-poap*pobp;
...
Qoaob=2*pd*(1-pd);
Qmuamub=Qkakb(mua,mub);

pmua=(poap*pobp*Qoaob+poap*pmub*Qoamub)/(pobp*Qmuaob+pmub*Qmuamub); 
pmuad=pmua_tot-pmua;

xoapobp=N*poap*pobp*Qoaob;
xoapmub=N*poap*pmub*Qoamub;
xmuaobp=N*pmua*pobp*Qmuaob;
xmuamub=N*pmua*pmub*Qmuamub;
xmaxA=max(xoapobp+xoapmub,xmuaobp+xmuamub);

xoaobd=N*pood*Qoaob;
xmuaobd=N*(pmuad*pob+pmua*pobd)*Qmuaob;
xmuamubd=N*pmuad*pmub*Qmuamub;
...
[~,Exmuaobu] = xtoxbzmg(xmuaobd,ebs);
[~,Exmuamubdu] = xtoxbzmg(xmuamubd,ebs);
[Exoanubl,~] = xtoxbzmg(xoanub,ebs);
[Exoadmubl,Exoadmubu] = xtoxbzmg(xoadmub,ebs);

EQoaobdu=Exoaobdu/(N*pood);
EQmuamubu=Exmuamubdu/(N*pmuad*pmub);
EQmuaobu=Exmuaobu/(N*(pmuad*pob+pmua*pobd));
EQoadmubu=Exoadmubu/(N*poad*pmub);
EQoadmubl=Exoadmubl/(N*poad*pmub);
EQoaobdl=Exoaobdl/(N*pood);

Ey01l=mub/(mub*nub-nub^2)/N*max(0,exp(nub)*Exoanubl/(poa*pnub)-nub^2/mub^2*exp(mub)*Exoadmubu/(poad*pmub)-(mub^2-nub^2)/mub^2*Exoaobdu/pood);
Ey10l=mua/(mua*nua-nua^2)/N*max(0,exp(nua)*Exnuaobl/(pnua*pob)-nua^2/mua^2*exp(mua)*Exmuaobu/(pmuad*pob+pmua*pobd)-(mua^2-nua^2)/mua^2*Exoaobdu/pood);


Es11zl=nz*(mua*mub*exp(-mua-mub)*Ey01l*Ey10l)/(EQoaobdu*EQmuamubu+EQmuaobu*EQoadmubu);
Es0mubzl=nz*(2*exp(-mua)*EQoaobdl*EQoadmubl)/(EQoaobdu*EQmuamubu+EQmuaobu*EQoadmubu);
...
Em2nu0l=1/2*N*pnua*pnub*delta*exp(-nua-nub)*EQoaobdl/pi;
Em02nul=1/2*N*pnua*pnub*delta*exp(-nua-nub)*EQoaobdl/pi;
[m02nul,~]=xbtoxzmg(Em02nul+Em2nu0l,ebs);

Em00u=1/2*N*pnua*pnub*exp(-2*(nua+nub))*EQoaobdu^2*qnuanubp/pi;
